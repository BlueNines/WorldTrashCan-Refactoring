...
version: 1.0.0
main: ai.wtc.fixture.WorldEntityLimitFixturePlugin
commands:
  worldentitylimitfixture:
    description: WorldListTrashCan world entity limit fixture
"""
...
def write_entity_limits(server_dir: Path, enabled: bool, ignored_worlds: list[str]) -> Path:
    ...
    target = server_dir / "plugins" / "WorldListTrashCan" / "entity-limits.yml"
    if not target.parent.is_dir():
        raise RuntimeError("WorldListTrashCan 配置目录尚未生成: " + str(target.parent))
...
def run_case(case: dict, run_root: Path, evidence_dir: Path, fixture_jar: Path) -> dict:
    ...
    with stdout_log.open("w", encoding="utf-8", errors="replace") as stdout, stderr_log.open("w", encoding="utf-8", errors="replace") as stderr:
        ...
        try:
            ...
            run_rcon(case, "wtc reload", responses, entries, "reload-seed-disabled")
            ...
            run_rcon(case, "wtc reload", responses, entries, "reload-active")
            ...
            run_rcon(case, "wtc reload", responses, entries, "reload-ignored")
...
def wait_indexed(case: dict, entries: list[str], minimum_entities: int) -> str:
    ...
    while time.time() < deadline:
        body = legacy.rcon_command(case["rcon"], "wtc debugdensity")
        last = body
        entries.append("> wtc debugdensity\n" + body.rstrip())
