...
完整功能矩阵文档也有独立审计脚本，检查 `docs/重构版完整功能与测试矩阵.md` 中的功能 ID 是否从 F-001 起连续、当前是否至少覆盖到 F-097、历史 `SKIP` 项是否全部写明后续收敛，以及“当前仍未收敛的通用专项项”是否保持为“无”：

```powershell
py -3 tools\rgb-visual-matrix\check_function_matrix_doc.py
```

当前矩阵为 F-001 到 F-097 共 97 个功能项，2026-06-08 历史 25 个 `SKIP` 通用专项项均已在后续专项中写明收敛，结果为 `errors: 0`。
...
当前审计覆盖 132 个正式 Java 源码文件，结果为 `errors: 0`。
...
当前审计覆盖 `tools/rgb-visual-matrix` 下 50 个 Python 脚本、23 个 `shutil.rmtree` 调用和 26 个 `unlink` 调用，结果为 `errors: 0`。
...
`cleanup.yml` 可以配置只在扫地时生效的强制直删世界列表：
...
`cleanup.yml` 可以选择保护掉落物实体所携带的、内部仍有物品的潜影盒 ItemStack：

```yaml
filled-shulker-boxes:
  # 是否保护掉落物携带的装有物品潜影盒；默认关闭。
  enabled: false
```

- 默认关闭，保持旧配置用户原有扫地行为；关闭时不会读取物品元数据或潜影盒内嵌库存。
- 开启后，扫地遇到 `Item` 实体时，只检查该实体的 `getItemStack()`，再读取该物品的 `BlockStateMeta` 内嵌库存；有内容的潜影盒本轮跳过并保留在地面。
- 空潜影盒不受保护，仍按普通物品进入世界、个人或公共垃圾桶；材质、名称和 Lore 保护规则也保持不变。
- 该功能只处理掉落物携带的潜影盒物品，**不读取也不保护世界中已经放置的潜影盒方块**；放置方块不属于扫地物品实体。
- 异常或无法安全读取内嵌库存时按保护处理，避免可能含物品的潜影盒被误删；配置变更后执行 `/wtc reload` 生效。

2026-08-03 使用同一个 universal 整包在 Paper 1.12.2 和 Folia 1.21.8 真实客户端完成默认关闭、开启保护、空潜影盒三轮对照。两端均得到：默认关闭装满潜影盒回收 1，开启保护装满潜影盒回收 0 且仍存在，开启保护空潜影盒回收 1；客户端截图、客户端日志、服务端日志和机器摘要已逐张复核。证据：`docs/test-evidence/filled-shulker-visual-20260803-171016/`。最终 universal SHA256：`0424efb4752abd4f1c9792b9dd81b5ec3999e735c9a0221e78e3bc1dddea8527`。
...
当前审计覆盖 28 个源码默认配置资源、5 个 dist jar、35 个包内配置资源和 1377 个 YAML 键，结果为 `errors: 0`。
...
当前审计覆盖 28 个源码默认配置资源、35 个 dist 包内默认配置资源和 154 个基准键，结果为 `errors: 0`。
...
2026-07-30 使用同一个 universal 整包在 Paper 1.12.2 与 Folia 1.21.8 完成真实客户端对照。三类垃圾桶全部启用且当前世界存在可用世界垃圾桶时，列表命中轮均显示 `回收物品 0（世界 0、个人 0、公共 0）`、`删除物品 7`；通过 `/wtc reload` 移出列表后，同位置 5 个物品均显示 `世界 5`、`删除物品 0`。本轮同时修正普通端路由分项过去按堆数统计的问题，现在普通端和 Folia 都按物品实际数量统计。脚本：`tools/rgb-visual-matrix/run_direct_remove_world_visual_matrix.py`；证据：`docs/test-evidence/direct-remove-world-visual-20260730-211433/`；被测 universal SHA256：`28df61b1e608b06648284fbb7e8152e5249413e6824dbad08108e947321ced24`。
...
- `docs/重构版完整功能与测试矩阵.md` 当前覆盖 F-001 至 F-097；正式命令入口为 `worldlisttrashcan/wtc`，公开权限为 `WorldListTrashCan.*`。历史 universal 整包矩阵及后续 GUI、世界垃圾桶、保护、实体限制、通知、Vault、公共动作按钮、鞍/主人保护、强制直删世界、移动物品保护和潜影盒物品保护专项共同构成当前回归基线。
...
- 2026-08-03 已使用同一个 universal 整包在 Paper 1.12.2 与 Folia 1.21.8 完成 F-097 潜影盒物品保护真实客户端验收。该功能只检查掉落物实体的 ItemStack，不检查世界方块；默认关闭时装满潜影盒回收 1，开启保护时装满潜影盒回收 0 且仍存在，开启保护时空潜影盒仍回收 1。两端三轮均 PASS，完整截图、客户端日志、服务端日志和机器摘要：`docs/test-evidence/filled-shulker-visual-20260803-171016/`；被测 universal SHA256：`0424efb4752abd4f1c9792b9dd81b5ec3999e735c9a0221e78e3bc1dddea8527`。
