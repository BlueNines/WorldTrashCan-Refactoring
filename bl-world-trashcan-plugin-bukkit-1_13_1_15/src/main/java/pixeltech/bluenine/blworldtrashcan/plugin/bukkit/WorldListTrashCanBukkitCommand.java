...
/** 新架构主命令，当前只提供架构验证命令。 */
public final class WorldListTrashCanBukkitCommand implements CommandExecutor, TabCompleter {
    ...
    private final WorldListTrashCanBukkitPlugin plugin;
    private final AddonCommandDispatcher addonCommands;

    /** 创建命令执行器。 */
    public WorldListTrashCanBukkitCommand(WorldListTrashCanBukkitPlugin plugin,
    ...
    /** 处理聊天、ActionBar 和 Title RGB/降级色通道测试命令。 */
    private void handleDebugRgbChannels(CommandSender sender, String[] args) {
        ...
        plugin.debugRgbChannels(player);
        sender.sendMessage("§a已发送 RGB/降级色通道测试: §f" + player.getName());
    }
