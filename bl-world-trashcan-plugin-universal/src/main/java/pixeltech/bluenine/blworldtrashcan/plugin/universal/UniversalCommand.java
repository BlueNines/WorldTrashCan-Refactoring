...
public final class UniversalCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleClear(CommandSender sender, String[] args) {
        ...
        if (plugin.isFoliaRuntime()) {
            handleFoliaClear(sender, ignoreGuards.booleanValue());
            return;
        }
        CleanupFeature.CleanupStats stats = plugin.runCleanupNow(ignoreGuards.booleanValue());
        sendClearResult(sender, stats);
    }

    /** 处理 Folia 立即清理命令。 */
    private void handleFoliaClear(CommandSender sender, boolean ignoreGuards) {
        ...
        if (!plugin.startCleanupNow(ignoreGuards)) {
    ...
    /** 处理打开公共垃圾桶。 */
    private void handleGlobal(CommandSender sender) {
    ...
    /** 处理打开个人垃圾桶。 */
    private void handlePersonal(CommandSender sender) {
        if (!requirePlayer(sender)) {
            return;
        }
    ...
    private void handleDebugWorldTrash(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(created
                ? (plugin.isFoliaRuntime() ? "§a已提交测试世界垃圾桶创建任务，请查看后台日志。" : "§a已创建并登记测试世界垃圾桶。")
                : "§c创建测试世界垃圾桶失败，请查看后台日志。");
    }
    ...
    private void handleDebugRoute(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(routed
                ? (plugin.isFoliaRuntime() ? "§a已提交路由测试任务，请查看后台日志。" : "§a路由测试成功。")
                : "§c路由测试失败，请检查容量、配置或世界垃圾桶登记。");
    }
    ...
    private void handleDebugDrop(CommandSender sender, String[] args) {
        ...
        boolean submitted = plugin.debugDrop(player, material, amount, markOwner);
        sender.sendMessage(submitted
                ? (plugin.isFoliaRuntime() ? "§a已提交测试掉落物生成任务，请查看后台日志。" : "§a已生成测试掉落物。")
                : "§c测试掉落物生成失败，请查看后台日志。");
    }
    ...
    private void handleDebugDamage(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(recovered
                ? (plugin.isFoliaRuntime() ? "§a已提交损坏回收测试任务，请查看后台日志。" : "§a损坏回收测试成功。")
                : "§c损坏回收测试未生效，请检查 damage-recovery 配置和垃圾桶容量。");
    }
    ...
    private void handleDebugRgbChannels(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(submitted
                ? "§a已发送 RGB 三通道测试: §f" + player.getName()
                : "§cRGB 三通道测试提交失败: " + player.getName());
    }

    /** 发送平台能力信息。 */
    private void sendPlatform(CommandSender sender) {
        sender.sendMessage(message("command.platform.header", "{prefix}&a当前平台: &f{platform}", "{platform}", plugin.getPlatform().id() + " (universal)"));
        for (Capability capability : Capability.values()) {
            ...
            sender.sendMessage(message("command.platform.line", "&7- &f{capability}&7: {state}",
                    "{capability}", capability.name().toLowerCase(Locale.ROOT).replace('_', '-'),
                    "{state}", state));
        }
    }
