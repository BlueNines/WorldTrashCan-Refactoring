...
/** 新架构主命令，当前只提供架构验证命令。 */
public final class WorldListTrashCanCommand implements CommandExecutor, TabCompleter {
    ...
    private final WorldListTrashCanPlugin plugin;
    private final AddonCommandDispatcher addonCommands;

    /** 创建命令执行器。 */
    public WorldListTrashCanCommand(WorldListTrashCanPlugin plugin,
    ...
    private void handleDebugRgb(CommandSender sender, String[] args) {
        ...
        sender.sendMessage("§a已发送 RGB 可见通道测试: §f" + player.getName());
    }
    ...
    private void handleDebugRgbChannels(CommandSender sender, String[] args) {
        ...
        sender.sendMessage("§a已发送 RGB 通道测试: §f" + player.getName());
    }
